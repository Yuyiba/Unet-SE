...
#losses_his=[]
writer = SummaryWriter('/home/yus/Documents/u_net_liver-master/tensorboard/6.14road/bs20_unet_GoOn')
...
def test():
    ...
    plt.ion()
    with torch.no_grad():
        for x, _ ,x_path in dataloaders:
            x_path=str(x_path).split("/")
            print(x_path[2][:-3])
            y=model(x)
            img_numpy = y[0].cpu().float().numpy()
            img_numpy = (np.transpose(img_numpy, (1, 2, 0))) / 1.0 * 255.0
...
if __name__ == '__main__':
    ...
    parse.add_argument("--batch_size", type=int, default=20) #6
